#from https://github.com/ultralytics/ultralytics/blob/main/ultralytics/utils/instance.py#L63
...
from computer_vision.yolov11.utils.ops import ltwh2xywh, ltwh2xyxy, resample_segments, xywh2ltwh, xywh2xyxy, xyxy2ltwh, xyxy2xywh
...
# `xyxy` means left top and right bottom, i.e., x1,y1,x2,y2 where (x1,y1) is top-left and < (x2,y2) bottom right
# `xywh` means center x, center y, width and height (YOLO format)
# `ltwh` means left top, width and height, i.e., x1,y1,w,h where (x1,y1) is top left
...
class Bboxes:
    """
    This class supports various bounding box formats like `xyxy`, `xywh`, and `ltwh` and provides methods for format conversion,
    scaling and area calculation. Bounding box data should be numpy arrays
    Notes:
    ...
    def convert(self, format:str)->None:
        """
        Convert bounding box format from one to another
        Args:
            format (str): Target format for conversion, one of `xyxy`, `xywh`, `ltwh`
        """
        assert format in _formats, f'Invalid bounding box format: {format}, format must be one of {_formats}'
