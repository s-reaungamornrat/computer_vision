...
class Annotator:
    ...
    def __init__(
        ...
        if self.pil:  # use PIL
            ...
            try:
                font = "Arial.Unicode.ttf" # check_font("Arial.Unicode.ttf" if non_ascii else font)
    ...
    def show(self, title: str | None = None):
        ...
        if IS_COLAB or IS_KAGGLE:  # can not use IS_JUPYTER as will run for all ipython environments
            try:
                ...
                warnings.warning(f"Unable to display image in Jupyter notebooks: {e}")
        else:
            im.show(title=title)
...
def plot_tune_results(csv_file: str = "tune_results.csv", exclude_zero_fitness_points: bool = True):
    ...
    def _save_one_file(file):
        ...
        plt.close()
        print(f"In utils.plotting.plot_tune_results -> Saved {file}")
...
def feature_visualization(x, module_type: str, stage: int, n: int = 32, save_dir: Path = Path("runs/detect/exp")):
    ...
    if isinstance(x, torch.Tensor):
        ...
        if height > 1 and width > 1:
            ...
            print(f"In utils.plotting.feature_visualization Saving {f}... ({n}/{channels})")
