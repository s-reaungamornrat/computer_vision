...
def masks2segments(masks:np.ndarray|torch.Tensor, strategy:str='all')->list[np.ndarray]:
    ...
    """
    from computer_vision.yolov11_pose.data.converter import merge_multi_segment
    ...
    for x in np.ascontiguousarray(masks):
        ...
        if c:
            if strategy=='all': # merge anc concatenate all segments
                c=(
        ...
        else: c=np.zeros((0,2)) # no segments found
