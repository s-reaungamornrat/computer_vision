...
from .ops import resample_segments, xywh2xyxy, xyxy2xywh, xyxy2ltwh, xywh2ltwh, ltwh2xyxy, ltwh2xywh
...
class Bboxes:
    ...
    The class supports various bounding box formats like `xyxy`, `xywh` and `ltwh` and provides methods for format conversion, scaling, and area 
    calculation. Bounding box data should be provided as numpy arrays.
    Examples:
    ...
    def __init__(self, bboxes: np.ndarray, format: str='xyxy')->None:
        ...
        Args:
            ...
            format (str): Format of the bounding boxes, one of 'xyxy', 'xywh', 'ltwh'
        """
    ...
    def convert(self, format:str)->None:
        ...
        if self.format=='xyxy': func=xyxy2xywh if format=='xywh' else xyxy2ltwh
        elif self.format=='xywh': func=xywh2xyxy if format=='xyxy' else xywh2ltwh
        else: func=ltwh2xyxy if format=='xyxy' else ltwh2xywh
    ...
    def areas(self)->np.ndarray:
        ...
        (self.bboxes[:,2]-self.bboxes[:,0])*(self.bboxes[:,3]-self.bboxes[:,1]) # format xyxy
            if self.format=='xyxy' 
            else self.bboxes[:,2]*self.bboxes[:,3] # format xywh and ltwh
        )
        
    def mul(self, scale:int|tuple|list)->None:
        """Multiply bounding box coodinates by scale factor(s)
        Args:
            scale (int|tuple|list): Scale factor(s) for four coordinates. If int, the same scale is applied to all
        """
        if isinstance(scale, Number): scale=to_4tuple(scale)
    ...
    def concatenate(cls, boxes_list:list[Bboxes], axis:int=0)->Bboxes:
        ...
        if not boxes_list: return cls(np.empty(0))
    ...
    def __getitem__(self,index:int|np.ndarray|slice)->Bboxes:
        ...
        """
        if isinstance(index, int): return Bboxes(self.bboxes[index].reshape(1,-1))
        b=self.bboxes[index]
...
class Instances:
    ...
    This class provides a unified interface for handling different types of object anntations including bounding boxes, segmentation masks, and keypoints.
    It supports various operations like scaling, normalization, clipping, and format conversion.
    
    Examples:
        Create instances with bounding boxes and segments
        >>> instances=Instances(bboxes=np.array([[10,10,30,30],[20,20,40,40]]),
    ...
    def __init__(self, bboxes:np.ndarray, segments:np.ndarray | list[np.ndarray]=None, keypoints:np.ndarray=None, bbox_format:str='xywh', 
        ...
        Args:
            ...
            segments (np.ndarray | list[np.ndarray], optional): Segmentation mask as a list of pointsets, each represent a border of 
                an object and each of shape (N, 2) for N points of (x, y)
            keypoints (np.ndarray, optional): Keypoints with shape (N,M,d) for example (N,17,3) for 17 human anatomical landmarks in (x,y,visibility)
        ...
        if segments is not None: assert isinstance(segments, (np.ndarray, list))
        if isinstance(segments, list): assert all(isinstance(s, np.ndarray) for s in segments)
    ...
    def denormalize(self, w:int, h:int)->None:
        ...
        Args:
            ...
            h (int): Image height
        """
        if not self.normalized: return
        self.scale(scale_w=w, scale_h=h, bbox_only=False)
    ...
    def normalize(self, w:int, h:int)->None:
        ...
        self.scale(scale_w=1/w, scale_h=1/h, bbox_only=False)
    ...
    def add_padding(self, padw:int, padh:int)->None:
        ...
        """
        assert not self.normalized, 'Adding padding only works on absolute coordinates'
    ...
    def __getitem__(self, index:int | np.ndarray | slice)->Instances:
        ...
        Args:
            index (int | np.ndarray | slice): The index, slice, boolean array to select the desired instances
        Returns:
        ...
        if len(self.segments):
            if isinstance(self.segments, np.ndarray): segments=self.segments[index]
            elif isinstance(self.segments, list): 
                if isinstance(index, (int, slice)): segments=self.segments[index]
                else: segments=[s for s, v in zip(self.segments, index) if v]
    ...
    def flipud(self, h:int)->None:
        ...
        if self.keypoints is not None: self.keypoints[...,1]=h-self.keypoints[...,1]
    ...
    def fliplr(self, w:int)->None:
        ...
        if self.keypoints is not None: self.keypoints[...,0]=w-self.keypoints[...,0]
    ...
    def clip(self, w:int, h:int)->None:
        ...
        if ori_format!='xyxy': self.convert_bbox(format=ori_format)
        if len(self.segments):
            if isinstance(self.segments, np.ndarray): 
        ...
        if self.keypoints is not None:
            # Set visibility of out-of-bound points to zero
            self.keypoints[...,2][
                (self.keypoints[...,0]<0) |
                (self.keypoints[...,0]>w) |
                (self.keypoints[...,1]<0) |
    ...
    def remove_zero_area_boxes(self)->np.ndarray:
        """Remove zero-area boxes, i.e., after clipping some boxes may have zeros width or height
        Returns:
            (np.ndarray): Boolean array indicating which boxes were to kept
        """
        good = self.bbox_areas > 0
        if not all(good):
            self._bboxes=self._bboxes[good]
            if len(self.segments):
                if isinstance(self.segments, np.ndarray): self.segments=self.segments[good]
                elif isinstance(self.segments, list):
                    self.segments=[s for v, s in zip(good, self.segments) if v]
            if self.keypoints is not None: self.keypoints=self.keypoints[good]
        return good
        
    def update(self, bboxes:np.ndarray, segments:np.ndarray|list[np.ndarray]=None, keypoints:np.ndarray=None):
        """Update instance variables
        Args:
            bboxes (np.ndarray): New bounding boxes, with shape (N,4)
            segments (np.ndarray | list[np.ndarray], optional): New segment
            keypoints (np.ndarray, optional): New keypoints
        """
        self._bboxes=Bboxes(bboxes, format=self._bboxes.format)
        if segments is not None: self.segments=segments
        if keypoints is not None: self.keypoints=keypoints
    ...
    def concatenate(cls, instances_list:list[Instances], axis=0)->Instances:
        """Concatenate a list of Instances objects into a single Instances objects.
        Args:
            instances_list (list[Instances]): A list of instances object to concatenate.
            axis (int, optional): The axis along which the arrays will be concatenated
        Returns:
            (Instances): A new Instances object containing the concatenated bounding boxes, segments, and keypoints if present
        Notes:
            The Instances object in the list should have the same properties, such as the format of the bounding boxes, whether
            keypoints are present, and if the coordinates are normalized
        """
        assert isinstance(instances_list, (list, tuple))
        if not instances_list: return cls(np.empty(0))
        ...
        else: cat_segments=np.concatenate([b.segments for b in instances_list], axis=axis)
