...
def non_max_suppression(
    ...
    Applies NMS to filter overlapping bounding boxes based on confidence and IoU thresholds. Support multiple detection formats including standard boxes,
    rotated boxes, and masks

    Args:
        prediction (torch.Tensor|tuple): Predictions with shape (batch_size, 4+num_classes+num_extra, num_boxes) containing boxes, class confidence, and extra
            if tuple, the first is output tensor
        conf_thres (float): Confidence threshold for filtering detection. Valid values are between 0. and 1.
        ...
        nc (int): Number of classes. Indices after this are considered extra
    ...
    Returns:
        output (list[torch.Tensor]): List of detections per image with shape (num_boxes,6+extra) containing (x1,y1,x2,y2,confidence,class,extra)
    ...
    # BCN prediction where C is  [4D bounding box, confidence, extra], where 4D is xywh
    if isinstance(prediction, (list, tuple)): prediction=prediction[0] # Bx56x8400
    if classes is not None: classes=torch.tensor(classes, device=prediction.device)
    
    if prediction.shape[-1]==6 or end2end: #end-to-end model (BNC, i.e., 1,300,6)
        output=[pred[pred[:,4] > conf_thres][:max_det] for pred in prediction]
        if classes is not None: output=[pred[(pred[:,5:6]==classes).any(1)] for pred in output]
        return output
    
    bs=prediction.shape[0] # batch size (BCN, i.e., 3,84,6300)
    nc=nc or (prediction.shape[1]-4) # number of classes
    extra=prediction.shape[1]-nc-4 # number of extra info
    mi=4+nc # mask start index
    xc=prediction[:,4:mi].amax(dim=1) > conf_thres # Bx1xN confidence
    xinds=torch.arange(prediction.shape[-1], device=prediction.device).expand(bs, -1)[...,None] # N -> BxN -> BxNx1

    # Settings
    # min_wh=2 # (pixels) minimum box width and height
    time_limit=2.+max_time_img*bs # seconds to quit after
    multi_label&=nc>1 # multiple labels per box (adds 0.5 ms/img)
    ...
    # BxNxC prediction, BxNx1 xinds
    for xi, (x, xk) in enumerate(zip(prediction, xinds)): # image-index, (preds, preds-indices)
        # Apply constraints
        filt=xc[xi] # xc is BxN masking elements passing condition that > conf_thresh
        x=x[filt] # NxC -> MxC where M<=N
        if return_idxs: xk=xk[filt] # NxC -> MxC where M<=N
    
        # If none remains, process next image
        if not x.shape[0]: continue
    
        # Detections matrix nx6 (xyxy, conf, extra)
        box, cls, mask=x.split((4, nc, extra), dim=1) # Mx4 box, Mx1 cls, MxE mask/extra
    
        if multi_label:
            i,j=torch.where(cls>conf_thres) # i and j are of size (M,)
            x=torch.cat((box[i], x[i,4+j,None],j[:,None].float(), mask[i]), dim=1) # Mx(4+1+1+E)
            if return_idxs: xk=xk[i]
        else: # best class only
            conf,j=cls.max(dim=1, keepdim=True) # confidence, indices, each of size (M,1)
            filt=conf.view(-1)>conf_thres
            x=torch.cat((box, conf, j.float(), mask), dim=1)[filt]
            if return_idxs: xk=xk[filt]
    
        # Filter by class
        if classes is not None:
            filt=(x[:,5:6]==classes).any(dim=1) # of size (M,1) -any-> (M,)
            x=x[filt]
            if return_idxs: xk=xk[filt]
        ...
        if rotated:
            ...
            else: i=TorchNMS.nms(boxes,scores,iou_thres)
    
        i=i[:max_det] # limit detections
        output[xi]=x[i]
        if return_idxs: keepi[xi]=xk[i].view(-1)
        if (time.time()-t)>time_limit: 
            warnings.warn(f"NMS time limit {time_limit:.3f}s exceeded")
